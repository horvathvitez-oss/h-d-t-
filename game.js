...
module.exports = function createGame(io, rawGameId, creatorUsername, howmany,
maplevel) {
...
room.players = createInitialPlayers(room.gameid, room.creatorUsername,
room.howmany);
...
emitSnapshot(room);
setTimeout(() => emitSnapshot(room, socket), 50);
return;
}
...
if (room.game.phase === PHASES.BATTLE_SELECTION || room.game.phase ===
PHASES.EXPANSION_SELECTION || room.game.phase === PHASES.BASE_SELECTION) {
...
room.game.currentOrder = room.game.orderCycle[room.game.expansionCycleIndex %
room.game.orderCycle.length] || [];
room.game.expansionCycleIndex = (room.game.expansionCycleIndex + 1) %
room.game.orderCycle.length;
room.game.pendingSelections = {};
room.game.reservedTerritories = [];
room.game.activeQuestion = null;
room.game.currentplayer = nextEligiblePidInOrder(room, room.game.currentOrder, -1,
true);
await persistGame(room);
...
gameLog(room, `Foglalási kör ${room.game.expansionRound}. Sorrend:
${formatOrder(room, room.game.currentOrder)}.`);
sendStatus(room, `Foglalási kör ${room.game.expansionRound}: ${current.name}
választ.`);
emitSnapshot(room);
}
...
const nextPid = nextEligiblePidInOrder(room, room.game.currentOrder, currentIndex,
false, room.game.pendingSelections);
...
const participants = Object.keys(room.game.pendingSelections).map((value) =>
Number(value));
...
room.game.currentOrder = room.game.orderCycle[(roundNumber - 1) %
room.game.orderCycle.length] || [];
room.game.currentplayer = nextEligiblePidInOrder(room, room.game.currentOrder, -1,
true);
room.game.activeQuestion = null;
...
gameLog(room, `Csatakör ${roundNumber}. Sorrend: ${formatOrder(room,
room.game.currentOrder)}.`);
emitSnapshot(room);
await maybeAdvanceWhenCurrentPlayerUnavailable(room);
}
...
room.game.baseOrder[room.game.baseSelectionIndex];
await persistGame(room);
emitSnapshot(room);
...
gameLog(room, `${current.name} kimarad ebből a csatakörből, mert nincs
kapcsolódva.`);
await advanceBattleTurn(room);
return;
}
...
gameLog(room, `${current.name} kimarad, mert nincs támadható szomszédos
ellenséges terület.`);
await advanceBattleTurn(room);
return;
}
sendStatus(room, `Csata ${room.game.battleRound}. kör: ${current.name} támadót
választ.`);
emitSnapshot(room);
}
}
...
gameLog(room, `${attacker.name} megtámadta ${territory.tname} területét. Védő:
${defender.name}.`);
...
const question = room.game.activeQuestion;
if (!question) return;
const everyoneAnswered = question.participants.every((pid) =>
Boolean(question.answers[pid]));
if (everyoneAnswered) {
...
const deadline = Date.now() + (config.type === 'mcq' ? MCQ_TIME_LIMIT_MS :
GUESS_TIME_LIMIT_MS);
...
resultLines.push(`${player.name} helyesen válaszolt, megszerezte:
${territory.tname}.`);
} else {
...
return {
pid,
distance: Number.isFinite(answer.distance) ? answer.distance :
Number.POSITIVE_INFINITY,
submittedAt: Number.isFinite(answer.submittedAt) ? answer.submittedAt :
Number.POSITIVE_INFINITY,
};
})
...
await persistTerritory(room, territory);
await persistAllPlayers(room);
resultLines.push(`${winner.name} szerezte meg a maradék területet:
${territory.tname}.`);
}
...
const { attackerPid, defenderPid, targetTid, isCastle, castleTid, castleStage } =
question.context;
...
if (attackerWins) {
await resolveSuccessfulAttack(room, { attackerPid, defenderPid, targetTid, isCastle,
castleTid, castleStage });
} else {
...
const attackerDistance = Number.isFinite(attackerAnswer.distance) ?
attackerAnswer.distance : Number.POSITIVE_INFINITY;
const defenderDistance = Number.isFinite(defenderAnswer.distance) ?
defenderAnswer.distance : Number.POSITIVE_INFINITY;
...
const line = `${attacker.name} elfoglalta ${territory.tname} területét ${defender.name}
játékostól.`;
gameLog(room, line);
...
const castle = room.castles.find((item) => item.tid === context.castleTid &&
item.active);
if (!castle) {
await advanceBattleTurn(room);
return;
...
attacker.castleCaptureBonus = (attacker.castleCaptureBonus || 0) +
CASTLE_SCORE_BONUS;
...
const line = `${attacker.name} lerombolta ${defender.name} várát, megszerezte az
összes területét, és ${defender.name} kiesett.`;
gameLog(room, line);
room.namespace.emit('battle:result', { message: line });
...
const line = `${attacker.name} újabb tornyot rombolt le ${defender.name} várából.
Maradék életerő: ${castle.hp}.`;
gameLog(room, line);
room.namespace.emit('battle:result', { message: line });
room.game.activeQuestion = null;
await persistGame(room);
...
async function resolveSuccessfulDefense(room, { attacker, defender, isCastle,
castleStage }) {
...
const currentIndex = room.game.currentplayer === -1 ? room.game.battleTurnIndex :
order.indexOf(room.game.currentplayer);
...
room.players.forEach((player) => {
const territoryCount = room.territories.filter((territory) => territory.ownsto ===
player.pid).length;
const activeCastleBonus = room.castles.some((castle) => castle.active && castle.pid
=== player.pid)
? CASTLE_SCORE_BONUS
: 0;
const assetScore = territoryCount * TERRITORY_SCORE + activeCastleBonus;
player.score = assetScore + (player.defenseBonus || 0) + (player.castleCaptureBonus ||
0);
});
}
...
function nextEligiblePidInOrder(room, order, currentIndex, includeCurrent = false,
selectionMap = null) {
...
return {
flow: 'EXPANSION',
pendingSelections: context.pendingSelections ? { ...context.pendingSelections } : {},
};
}
return {
flow: 'BATTLE',
...
return `${player ? player.name : 'Ismeretlen'}: ${answer.guess}${question.unit ? ' ' +
question.unit : ''}`;
});
}
...
let question = MULTIPLE_CHOICE_QUESTIONS[Math.floor(Math.random() *
MULTIPLE_CHOICE_QUESTIONS.length)];
while (room.usedMcqIds.has(question.id)) {
question = MULTIPLE_CHOICE_QUESTIONS[Math.floor(Math.random() *
MULTIPLE_CHOICE_QUESTIONS.length)];
}
...
let question = GUESS_QUESTIONS[Math.floor(Math.random() *
GUESS_QUESTIONS.length)];
while (room.usedGuessIds.has(question.id)) {
question = GUESS_QUESTIONS[Math.floor(Math.random() *
GUESS_QUESTIONS.length)];
}
...
await Gameutils.findOneAndUpdate({ gameid: room.gameid }, room.game, { new: true,
upsert: true });
}

async function persistPlayer(room, player) {
await Player.findOneAndUpdate({ gameid: room.gameid, pid: player.pid }, player, { new:
true, upsert: true });
}
...
await Territory.findOneAndUpdate({ gameid: room.gameid, tid: territory.tid }, territory, {
...
await Castle.findOneAndUpdate({ gameid: room.gameid, tid: castle.tid }, castle, { new:
true, upsert: true });
}
...
const T = (tid, continent, tname, neighbors) => ({ gameid, tid, continent, tname,
neighbors, ownsto: -1 });
